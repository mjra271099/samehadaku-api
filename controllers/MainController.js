...
class MainController {
  home({ params: { page } }, req) {
    ...
    scraperjs.StaticScraper.create(`https://samehadaku.email/${page}`).scrape(
      async ($) => {
        const obj = {}
        obj.season = $('.animposx')
          .map(function () {
            return {
              ...
              linkId: $(this)
                .find('a')
                .attr('href')
                .replace('https://samehadaku.email/anime/', '')
                .replace('/', ''),
              image: $(this)
  ...
  blog({ params: { page } }, req) {
    ...
    scraperjs.StaticScraper.create(
      `https://samehadaku.email/blog/${page}`,
    ).scrape(($) => {
      const blog = {}
      blog.blog = $('.box-blog')
        .map(function () {
          return {
            ...
            linkId: $(this)
              .find('.img a')
              .attr('href')
              .replace('https://samehadaku.email/blog/', '')
              .replace('/', '')
              .trim(),
  ...
  readblog({ params: { id } }, req) {
    const page = `https://samehadaku.email/blog/${id}`
  ...
  tag({ params: { tag } }, req) {
    const page = `https://samehadaku.email/tag/${tag}`
  ...
  blogcategory({ params: { category } }, req) {
    const page = `https://samehadaku.email/blog-category/${category}`
  ...
  blogCategoryByPage({ params: { category, page } }, req) {
    const pager = `https://samehadaku.email/blog-category/${category}/page/${page}`
  ...
  daftarGenre(_, req) {
    scraperjs.StaticScraper.create('https://samehadaku.email/').scrape(($) => {
      const obj = {}
      obj.daftar_genere = $('.genre > li').map(function () {
        ...
        return {
          ...
          link: $(this).find('a').attr('href'),
          linkid: $(this).find('a').attr('href').replace('https://samehadaku.email/genre/', ''),
          total: span,
        }
      }).get()
      req.send(obj)
  ...
  anime({ params: { id } }, req) {
    const page = `https://samehadaku.email/anime/${id}/`
    scraperjs.StaticScraper.create(page).scrape(async ($) => {
        ...
        .map(function () {
          return {
            ...
            id: $(this)
              .find('.epsright .eps a')
              .attr('href')
              .replace('https://samehadaku.email/', ''),
          }
        })
        .get()
      ...
      await scraperjs.StaticScraper.create('https://samehadaku.email/').scrape(
        async ($) => {
  ...
  async readanime({ params: { link } }, req) {
    const page = `https://samehadaku.email/${link}/`
  ...
  search({ params: { title } }, req) {
    const page = `https://samehadaku.email/?s=${title}`
  ...
  searchByPage({ params: { title, page } }, req) {
    const pager = `https://samehadaku.email/page/${page}/?s=${title}`
  ...
  season(_, req) {
    const page = 'https://samehadaku.email/season/spring-2020/'
    ...
    scraperjs.StaticScraper.create(page)
      ...
      .then((data) => {
        const page = 'https://samehadaku.email/season/spring-2020/page/2/'
  ...
  date(_, req) {
    const page = 'https://samehadaku.email/jadwal-rilis/'
  ...
  listWithoutPage(_, req) {
    const page = 'https://samehadaku.email/daftar-anime-2/'
  ...
  listWithPage({ params: { page } }, req) {
    const inPage = `https://samehadaku.email/daftar-anime-2/page/${page}/`
  ...
  searchByGenre({ params: { genre } }, req) {
    const page = `https://samehadaku.email/genre/${genre}`
